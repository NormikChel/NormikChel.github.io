...
// Объекты с текстами на разных языках
const texts = {
    ru: {
        welcome: "Добро пожаловать в мир Minecraft!",
        about: "О игре",
        aboutText: "Minecraft — это замечательная игра в жанре песочница, где вы можете строить, исследовать и выживать в бесконечном мире. Создавайте свои собственные приключения и делитесь ими с друзьями! К сожалению, в этой процветающей игре есть и «НО», которые можно узнать в Internet.",
    ...
    en: {
        ...
        aboutText: "Minecraft is a wonderful sandbox game where you can build, explore, and survive in an endless world. Create your own adventures and share them with friends! Unfortunately, in this thriving game, there are some \"BUTs\" that can be found on the Internet.",
        features: "Features",
        featureList: [
...
function updateText() {
    document.getElementById('welcome-text').textContent = texts[currentLanguage].welcome;
    document.getElementById('about-header').textContent = texts[currentLanguage].about;
    document.getElementById('about-text').textContent = texts[currentLanguage].aboutText;
    const featureList = texts[currentLanguage].featureList;
    ...
    document.getElementById('download-header').textContent = texts[currentLanguage].download;
    document.getElementById('download-text').textContent = texts[currentLanguage].downloadText;
    document.getElementById('gallery-header').textContent = texts[currentLanguage].gallery;
    document.getElementById('reviews-header').textContent = texts[currentLanguage].reviews;
    document.getElementById('review1').textContent = texts[currentLanguage].review1;
    document.getElementById('review2').textContent = texts[currentLanguage].review2;
    document.getElementById('footer-disclaimer').textContent = texts[currentLanguage].footerDisclaimer;
}
